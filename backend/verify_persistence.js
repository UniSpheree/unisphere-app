...
try {
  ...
  const events = db.prepare('SELECT id, title, organizer_email, date FROM events').all();
  ...
  events.forEach(e => console.log(`    - ${e.title} by ${e.organizer_email} on ${e.date?.split('T')[0]}`));
  
  console.log(`  Tickets: ${tickets.length}`);
  tickets.forEach(t => console.log(`    - ${t.user_email} → ${t.title}`));
  
  console.log('\n✅ All data is properly saved and persistent!');
  console.log('   Reload the app and all data will be restored automatically.\n');
